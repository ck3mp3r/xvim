...
let
  ...
  lsp = import ./lsp.nix { inherit (pkgs) vimPlugins; };
  ...
  keys = import ./keys.nix {
    ...
    inherit helpers;
    registrations = git.registrations // bufferline.registrations
      // direnv.registrations // lsp.registrations // obsidian.registrations
      // telescope.registrations // toggleterm.registrations
...
in {
  config = {
    ...
    keymaps = keys.bindings ++ bufferline.bindings ++ direnv.bindings
      ++ git.bindings ++ lsp.bindings ++ obsidian.bindings ++ telescope.bindings
      ++ toggleterm.bindings ++ zenmode.bindings;
  };
}
